...
PSP0.1 sobre uno o varios archivos fuente `.java`. La ejecución principal usa una
interfaz de terminal que permite escribir una o varias rutas de archivos a
analizar. También conserva el modo por argumentos para pruebas rápidas. Para cada
archivo recibido, el programa reporta:
...
| --- | --- |
| `App.java` | Punto de entrada; usa argumentos directos o abre el menú interactivo. |
| `InterfazTerminal.java` | Menú de terminal para capturar rutas, mostrar ayuda y regresar al inicio ante errores. |
| `AnalizadorLOC.java` | Orquestador del análisis; valida rutas, coordina el análisis e imprime resultados amigables. |
...
Desde la carpeta `Programa2/PSP1`, ejecute el menú interactivo:

```bash
java App
```

En el menú seleccione la opción `1` y escriba una o varias rutas separadas por
coma, por ejemplo:

```text
ProgramaPrueba1.java, ProgramaPrueba2.java
```

También se conserva el modo directo por argumentos para automatización o pruebas:
...
Salida obtenida al ejecutar los casos de prueba incluidos desde el menú con la
entrada `ProgramaPrueba1.java, ProgramaPrueba2.java`:

```text
=== Analizador LOC PSP0.1 ===
1. Analizar archivo(s) Java
2. Ver instrucciones
3. Salir
Seleccione una opcion:

Ingrese una o varias rutas .java separadas por coma.
Ejemplo: ProgramaPrueba1.java, ProgramaPrueba2.java
Ruta(s):
...
Analisis finalizado. Regresando al menu principal.
```

## 11. Manejo amigable de excepciones

Cuando ocurre un problema, el programa no imprime trazas técnicas. En su lugar,
muestra un mensaje claro con la ruta afectada, el motivo del error y regresa al
menú principal para que el usuario pueda corregir la entrada. Por ejemplo, si se
ingresa `NoExiste.java`, la salida es:

```text
No fue posible analizar el archivo: NoExiste.java
Motivo: El archivo no existe.
Revise la ruta e intente nuevamente desde el menu principal.
```
...
## 13. Justificación de refactorización

Se actualizó `App.java` para abrir una interfaz de terminal cuando no se reciben
argumentos y se agregó `InterfazTerminal.java` para manejar el ciclo de menú, la
captura de rutas y los errores amigables. `AnalizadorLOC` se refactorizó para
exponer `procesarArchivos`, validar extensiones `.java` y reportar fallos por
archivo sin finalizar abruptamente. La solución se dividió en clases pequeñas para
mejorar claridad, mantenibilidad y exactitud:
...
- `InterfazTerminal` separa la interacción con el usuario del análisis técnico.
